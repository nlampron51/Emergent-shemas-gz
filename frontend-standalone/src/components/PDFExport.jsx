...
import { mockUnits, mockResources, mockSchedule } from '../data/mockData';
...
const PDFExport = () => {
  ...
  const handleExportPDF = () => {
    ...
    toast({
      title: "Export PDF simulé",
      description: `PDF généré avec succès pour ${selectedUnitsList.length} unités. Dans une vraie application, le fichier serait téléchargé.`,
    });

    // Dans une vraie application, vous pourriez utiliser une bibliothèque comme jsPDF
    // ou générer un PDF côté serveur
    console.log('Export PDF avec options:', { exportOptions, selectedUnits: selectedUnitsList.map(u => u.id) });
  };
  ...
  const generatePreview = () => {
    ...
    toast({
      title: "Aperçu généré",
      description: `Aperçu du PDF avec ${selectedUnitsList.length} unités sélectionnées.`,
    });
  };
